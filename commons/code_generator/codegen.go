...
func GetGenerateStruct(s interface{}) GenerateStruct {
	structName := commons.StructName(s)
	structFields := commons.StructFields(s)
...
func generateController(baseDir, pkgName string, s GenerateStruct) error {
	...
	p, err := getFilePath(baseDir, "/web/api/"+strcase.ToSnake(s.Name+"_controller.go"))
	if err != nil {
		return err
	}
	return writeFile(p, c)
...
func writeFile(filepath string, content string) error {
	exists, err := commons.PathExists(filepath)
	if err != nil {
		return err
	}
	if exists {
	...
	return commons.WriteString(filepath, content, true)
}
